...
public class Main extends Application {
	...
	public void start(Stage primaryStage) throws Exception {
		...
		GridPane.setConstraints(lbl_selectMovie, 5,  1);
			...
			lbl_description.getStyleClass().add("");
			GridPane.setConstraints(lbl_description, 5, 5);
		...
		btn_purchase.setOnAction(e -> {
			...
			if(isWed == "Wednesday") {				
				if(cbxMovies.getValue() == null) {
					Alert alert = new Alert(AlertType.INFORMATION);
					String a = "Please Select a Movie!";
					...
					totalPrice = ((childTicketTotal + studentTicketTotal + adultTicketTotal + oapTicketTotal) - (discountFactor * totalTickets));
					...
					alert.setHeaderText("Total for the tickets purchased (including Wednesday Discount)");
					alert.showAndWait();
				}
			}
			else {
		...
		GridPane.setConstraints(btn_reset, 1, 49);
		
		btn_purchase.getStyleClass().add("btn_purchase");
		GridPane.setConstraints(btn_purchase, 55, 49);
